...
import com.cliprelay.ui.MainScreen
...
class MainActivity : ComponentActivity() {
        ...
        setContent {
            AppTheme(useDarkTheme = false) {
                MainScreen(
                    ...
                    onResumeSync = { sendAction(ClipRelayService.ACTION_RESUME_SYNC) },
                    onScanNow = {
                    ...
                    onActionPauseSync = {
                        sendAction(ClipRelayService.ACTION_PAUSE_SYNC)
                        refreshDashboardState()
                    },
                    onActionDisconnectAll = {
                        sendAction(ClipRelayService.ACTION_DISCONNECT_ALL)
                        refreshDashboardState()
                    },
                    onActionStopService = {
                    ...
                    onApplyClipboard = { entry ->
                        val svc = Intent(this, ClipRelayService::class.java).apply {
    ...
    private fun rebuildFeed() {
        feed.value = ClipRelayService.getFeedSnapshot()
    }
    ...
    private fun handlePairingInput(input: String): Boolean {
        ...
        val ctx = this
        if (cleaned.startsWith("deskdrop://pair") || cleaned.startsWith("cliprelay://pair")) {
