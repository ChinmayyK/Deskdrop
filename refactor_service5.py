...
def refactor_line_by_line():
    ...
    for i, line in enumerate(lines):
        ...
        # 6. Skip activeTransfers private map
        if line.strip() == 'private val activeTransfers = mutableMapOf<String, TransferProgress>()':
            continue # single line
            
        # 7. publishActiveTransfers block
        if line.strip() == 'private fun publishActiveTransfers() {':
            skip = True
        ...
        if skip:
            if line.startswith('}'):
                ...
                if i > 0 and '        }' in lines[i] and 'activityFeed.toList()' in lines[i-1]:
                    skip = False
                    continue
                if i > 0 and 'activeTransfersFlow.value =' in lines[i-1]:
                    skip = False
                    continue

            # TransferProgress ends with ) instead of }
            if line.startswith(')'):
                if i > 0 and 'val isOutbound: Boolean' in lines[i-1]:
                    skip = False
                    continue
