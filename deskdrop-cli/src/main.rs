//! deskdrop-cli — live daemon control + product command surface.
...
async fn run() -> Result<()> {
    ...
    match refs.as_slice() {
        ...
        ["ping"] => cmd_ping().await,
        ["version"] | ["--version"] | ["-V"] => {
            println!("deskdrop-cli {}", env!("CARGO_PKG_VERSION"));
            Ok(())
        }
        ...
        other => bail!(
            "Unknown command: '{}'\n\nRun `deskdrop-cli help` to see all available commands.",
            other.join(" ")
        ),
    }
}
    ...
    match ipc(&IpcRequest::TemplateList).await? {
        IpcResponse::Ok { data: Some(data) } => {
            ...
            if templates.is_empty() {
                println!("No templates configured. Add one with: deskdrop-cli template add <name> <text>");
                return Ok(());
            }
...
fn print_help() {
    println!(
        r#"deskdrop-cli {}  —  Deskdrop management tool

USAGE:  deskdrop-cli <command> [args]
        deskdrop-cli /history            (leading slash prefix accepted)
        deskdrop-cli /send <device> "<text>"
...
EXAMPLES
  deskdrop-cli /history
  deskdrop-cli history --search "github"
  deskdrop-cli history --type text --last 10
  deskdrop-cli history --device iphone
  deskdrop-cli history stats
  deskdrop-cli history tag 42 work
  deskdrop-cli history export json > backup.json
  deskdrop-cli template add email "alice@example.com"
  deskdrop-cli template push email
  deskdrop-cli send macbook "meeting at 3pm"
  deskdrop-cli history repush 42 windows-pc
  deskdrop-cli devices peer-settings <uuid> pause
  deskdrop-cli settings set sync_urls_only true
  deskdrop-cli settings set min_text_length 3
  deskdrop-cli events --last 10
"#,
