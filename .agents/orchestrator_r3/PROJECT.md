...
| M2 | Android MediaStore & Query Optimization | Optimize `RemoteFileManager.kt` & `DeskdropService.kt` with SQL selection filters, fast summary counts, and pagination | None | PLANNED |
| M3 | RPC Protocol & Dynamic Timeout Hardening | Update `ipc.rs` & `engine/mod.rs` to support configurable timeouts, error fast-path on disconnect, and pagination handling | M1 | PLANNED |
| M4 | C FFI Export & Swift/WinUI Integration | Expose `deskdrop_send_remote_files_response` in `ffi.rs` and update Swift & C# FFI layers | M1 | PLANNED |
